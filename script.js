...
(() => {
  ...
  //    Requires:
  //    - header has id="siteHeader"
  //    - button has id="navToggle"
  //    - mobile panel has id="mobileNav"
  // -------------------------------
  (function mobileNav() {
  ...
  //    (keeps your behavior, but avoids hijacking empty/# links)
  // -------------------------------
  ...
  //    - If IntersectionObserver missing, show all immediately.
  //    - Uses class ".is-visible" by default.
  //    - Also adds ".show" for backward compatibility with your CSS.
  // -------------------------------
  const revealEls = qsa(".reveal");
  const revealKidsClass = ".reveal-child";

  const makeVisible = (el) => {
    el.classList.add("is-visible");
    el.classList.add("show"); // keep compatible with your older CSS
  };

  // if no IO support -> reveal everything
  if (!("IntersectionObserver" in window)) {
    revealEls.forEach((el) => {
      makeVisible(el);
      qsa(revealKidsClass, el).forEach((kid) => makeVisible(kid));
    });
  } else {
    const revealObserver = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => {
          ...
          // stagger children
          const kids = qsa(revealKidsClass, el);
          if (kids.length) {
            kids.forEach((k, i) => {
  ...
  // -------------------------------
  // 3) Count-up animation (your original, safe)
  // -------------------------------
  ...
  if (heroCard) {
    ...
    // subtle breathing on touch devices (only if user stays on page)
    if (window.matchMedia("(hover: none)").matches) {
      let t = 0;
      setInterval(() => {
        // if element removed or hidden, avoid work
  ...
  // -------------------------------
  // 8) Scroll-based background intensity
  // -------------------------------
  ...
  (function initTimelineProgress(){
    const prog = qs("#timelineProgress");
    ...
    function update() {
      ...
      const items = qsa(".t-item", wrap);
      const activeCount = Math.floor(clamped * items.length);
      items.forEach((it, i) => {
        const dot = qs(".t-dot", it);
        if (!dot) return;
        dot.style.background = i <= activeCount
          ? "rgba(76,67,177,0.90)"   // matches your Onix palette
          : "rgba(76,67,177,0.35)";
      });
    }
  ...
  // -------------------------------
  // Booking link: bind all CTAs (optional)
  // If window.BOOKING_URL exists, use it.
  // Otherwise leave your existing hrefs.
  // -------------------------------
  ...
  (function cookieBanner(){
    const KEY = "onix_cookie_choice"; // "accepted" | "declined"
